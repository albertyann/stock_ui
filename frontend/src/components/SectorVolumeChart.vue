<template>
  <el-card v-loading="loading" class="sector-volume-card">
    <template #header>
      <div class="card-header">
        <div class="header-left">
          <span class="card-title">板块成交量（近20交易日）</span>
    ...
    <div ref="chartRef" class="sector-volume-chart"></div>
  </el-card>
</template>
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await watchlistApi.getSectorVolume(null, 20)
    if (!res.success || !res.data) {
      ElMessage.info('暂无板块成交量数据')
      return
    }
    ...
    console.error('Failed to fetch sector volume data:', error)
    ElMessage.error('加载板块成交量数据失败')
  } finally {
    loading.value = false
  }
}
      ...
      formatter: function(params) {
        ...
        sortedParams.forEach(p => {
          ...
          html += `<div style="display:flex;justify-content:space-between;align-items:center;gap:20px;margin:3px 0;">
            ...
            <span style="font-weight:600;">${formatVolume(p.value)}</span>
          </div>`
        })
        return html
      }
